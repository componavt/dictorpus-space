...
def analyze_meanings_without_concept(
    ...
    """Find meanings that lack concept_id, grouped by meaning_ru.

    Returns DataFrame sorted by frequency descending:
        meaning_ru, meaning_ru_norm, count, langs, pos_values, sample_ids
    """
    df = meanings_df.copy()
    ...
    if without.empty:
        return pd.DataFrame(
            columns=["meaning_ru", "meaning_ru_norm", "count", "langs", "pos_values", "sample_ids"]
        )
...
def analyze_concept_usage(
    ...
    # Enrich with concept catalog info if available
    if concepts_wdh_df is not None:
        concept_info = concepts_wdh_df[
            ["concept_id", "category_id", "concept_ru", "concept_en",
             "wdh", "wdh_source", "wdh_confidence"]
        ].copy()
        usage = usage.merge(concept_info, on="concept_id", how="left")

    usage = usage.sort_values("meaning_count", ascending=False).reset_index(drop=True)
    return usage
...
def run_gap_audit(
    ...
    print(f"  {len(no_concept)} unique Russian glosses without concept assignment")
    if len(no_concept) > 0:
        print(f"  Top 5: {no_concept.head(5)[['meaning_ru', 'count']].to_dict('records')}")

    # 2. Concept usage
    print("Analyzing concept usage...")
    usage = analyze_concept_usage(meanings_df, concepts_wdh_df)
    reports["concept_usage"] = usage
    usage.to_csv(out / "audit_concept_usage.csv", index=False)
    print(f"  {len(usage)} concepts in use")
    if len(usage) > 0:
        unused_in_catalog = 0
        if concepts_wdh_df is not None:
            catalog_ids = set(concepts_wdh_df["concept_id"].astype(str))
            used_ids = set(usage["concept_id"].astype(str))
