1) При сохранении результатов (17 полей) для каждого отдельного прогона с LLM-переводчиком мы получаем отдельный файл.  
Поэтому `model_key`, `model_name`, `backend_family` нет смысла записывать десятки тысяч раз одинаковые в один и тот же файл. Эту информацию мы укажем либо в названии файла, либо в имени папки, либо где-либо ещё. 

2) `translation_input_mode` - совершенно не понял что это и зачем? Нужно твоё пояснение, лучше с примерами. 

3) `input_text_used` - точно лишнее, если известны `pos` и `meaning_ru`.

4) `meaning_ru_back` - не понятно: это присутствует не у всех LLM-переводчиков? Но при этом должно существовать во всех файлах?

5) Удаляем, как избыточные поля: `is_single_word_ru`, `input_token_count`, `output_token_count`, `qa_version`. 

====================

Укажи точечные изменения в прилагаемом файле `src/sem_cat/README.md`, отражающие новую реальность в шаге `01 reuse_analysis`. Такжи кратко укажи в этой документации команды для тестирования этого шага. Точечно, то есть укажи конкретно: был старый фрагмент, вот новый фрагмент.
